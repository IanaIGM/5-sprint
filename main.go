...
func (t Training) meanSpeed() float64 {
	...
	distance := t.distance()
	return distance / float64(t.Duration.Hours())
}
...
func (r Running) Calories() float64 {
	caloriesCount := ((CaloriesMeanSpeedMultiplier*float64(r.meanSpeed()) + CaloriesMeanSpeedShift) * r.Weight / MInKm * float64(r.Duration.Hours()) * MinInHours)
...
func (w Walking) Calories() float64 {
	// вставьте ваш код ниже
	speedInMsec := w.meanSpeed() * KmHInMsec       // расчёт значение средней скорости движения в м/с
	averageSpeedInMsec := math.Pow(speedInMsec, 2) // возведение скорость движения в м/с в квадрат
	heightInM := w.Height / 100
	caloriesCount := ((CaloriesWeightMultiplier*w.Weight + (averageSpeedInMsec/heightInM)*CaloriesSpeedHeightMultiplier*w.Weight) * float64(w.Duration.Hours()) * MinInHours)
	return caloriesCount
}
...
func (s Swimming) meanSpeed() float64 {
	...
	swimDistance := s.distance()
	return swimDistance / float64(s.Duration.Hours())
}
...
func (s Swimming) Calories() float64 {
	// вставьте ваш код ниже
	swimmingSpeed := float64(s.meanSpeed()) + SwimmingCaloriesMeanSpeedShift
	countCalories := swimmingSpeed * SwimmingCaloriesWeightMultiplier * s.Weight * float64(s.Duration.Hours())
